...
Beyond diagnosis, \method also includes a biome-aware debiasing framework under the same frozen-backbone linear-probing protocol. We 
% use biome-balanced ERM as a simple mitigation baseline and
define stronger biome-aware strategies, including  Dynamic Biome Reweighting with Validation Feedback (DBR), and Biome-Orthogonal Linear Probing (BOLP). BOLP removes biome-associated directions from frozen RSFM embeddings before training the task head, providing a representation-level debiasing strategy. This allows us to analyze whether biome-dependent disparities can be reduced without full backbone fine-tuning.
...
\begin{itemize}[leftmargin=*]
    ...
    \item We develop and evaluate biome-aware mitigation strategies, including Dynamic Biome Reweighting with Validation Feedback (DBR) and Biome-Orthogonal Linear Probing (BOLP), showing that mitigation can improve worst-group robustness but may trade off aggregate accuracy depending on the task.
\end{itemize}
...
For MMEarth20K, we use a 20,000-sample subset with Dynamic World-derived 10 m land-cover label maps. The subset is split into 16,000 training samples, 2,000 validation samples, and 2,000 test samples, enabling biome-aware dense land-cover evaluation under the same frozen-backbone protocol. To rigorously evaluate group robustness, we curated this subset via stratified sampling to enforce a controlled inter-group imbalance while maintaining uniform sample frequencies for the raw biomes within each macro-group
...
% \scalebox{0.8}{
\caption{Mapping from 14 terrestrial biome classes to six biome macro-groups used for primary fairness evaluation and debiasing.}
...
{\small 7 Tropical and Subtropical Grasslands, Savannas and Shrublands; 8 Temperate Grasslands, Savannas and Shrublands; 12 Mediterranean Forests, Woodlands and Scrub} &
...
Unknown/water-only, or unmatched samples are excluded from the primary fairness summaries and worst-group scores. Some datasets cover only a subset of the six macro-groups, and therefore all fairness metrics are computed over the set of matched macro-groups present in each dataset split.
...
While the raw biome label \(b_i\) enables fine-grained ecological analysis for future dataset users, the macro-group label \(g_i\) is used for the primary fairness metrics and mitigation methods in our experiments. This separation allows \method to preserve ecological detail without making the primary robustness analysis overly sensitive to sparse biome classes.
...
For segmentation, we compute EOdd and DPM analogously at the pixel level by aggregating class-wise true-positive rates, false-positive rates, and predicted-positive rates over all pixels from patches assigned to each biome macro-group.
...
\end{equation}
 DBR is lightweight, reuses validation metrics already computed for fairness evaluation, is non-adversarial, introduces no trainable parameters beyond the task head, and applies to single-label classification, multi-label classification, and segmentation tasks.
...
For the Dynamic Biome Reweighting (DBR) experiments, we set the weight update frequency $K=1$, feedback strength $\alpha=0.1$, and stability constant $\epsilon=10^{-5}$.
...
Table~\ref{tab:per_biome_classification} reports macro-group-wise classification and segmentation results for Prithvi-EO-2.0 under original ERM, BOLP, and DBR. On m-EuroSAT, the Aseasonal High-Biomass group is the weakest under original ERM (0.836 Macro-F1), despite strong aggregate performance. On m-BigEarthNet, the Transitional Herbaceous and Scrub group obtains the lowest original F1@opt (0.497), while the forest-dominated groups perform better. These results show that high aggregate classification performance can hide substantial ecological group disparities.
...
& Transitional Herbaceous and Scrub & 875 & 0.289 & 0.279 & 0.269 \\
...
The segmentation tasks show similar group-dependent behavior. On m-SA-Crop-Type, the Xeric and Mineralogical group has much lower original mIoU (0.176) than the Transitional Herbaceous and Scrub group (0.289). On MMEarth20K, the Hydrologically Modulated group is the weakest under original ERM (0.333 mIoU). These patterns confirm that biome-dependent disparities are present not only in classification but also in dense prediction tasks.

\begin{table*}[h]
\centering
\caption{Debiasing and robustness summary for classification and segmentation tasks under the frozen-backbone Prithvi-EO-2.0 linear-probing protocol. Overall denotes Macro-F1 for m-EuroSAT, F1@opt for m-BigEarthNet, and global mIoU for segmentation datasets. Worst denotes the corresponding worst-group score.}
...
\cmidrule{2-9}
 ...
 & m-BigEarthNet & DBR & 0.600 & 0.546 & 0.280 & 0.007 & 0.441 & 0.138 \\
\midrule
...
\cmidrule{2-9}
 ...
 & MMEarth20K & BOLP & 0.437 & 0.334 & 0.294 & 0.040 & 0.173 & 0.125 \\
...
Table \ref{tab:debiasing_results_cls} summarizes the biome-aware robustness and mitigation results under the frozen-backbone Prithvi-EO-2.0 linear-probing protocol. Lower NFR, standard deviation, EOdd, and DPM indicate more consistent performance across biome groups.


On the classification tasks, BOLP improves worst-group performance on both m-EuroSAT and m-BigEarthNet. For m-EuroSAT, BOLP improves the worst-group score from 0.836 to 0.843 and slightly reduces NFR from 0.104 to 0.100. For m-BigEarthNet, BOLP improves overall F1@opt from 0.626 to 0.642 and worst-group F1@opt from 0.497 to 0.520, while also reducing EOdd from 0.564 to 0.392 and DPM from 0.215 to 0.138. DBR provides a different trade-off: it obtains the highest worst-group score on both classification datasets, but reduces overall F1@opt on m-BigEarthNet.

For segmentation, the mitigation behavior is more task dependent. On m-SA-Crop-Type, both BOLP and DBR improve worst-group mIoU over original ERM, from 0.176 to 0.195 and 0.197, respectively, but reduce global mIoU. On MMEarth20K, DBR slightly improves both overall mIoU and worst-group mIoU. BOLP also slightly improves worst-group mIoU, but reduces overall mIoU. These results indicate that biome-aware mitigation can improve ecological robustness, but the trade-off between aggregate accuracy and worst-group performance depends on the task and dataset.
...
The segmentation results are more mixed. On m-SA-Crop-Type, BOLP and DBR improve worst-group mIoU but reduce global mIoU; on MMEarth20K, DBR gives the most balanced outcome, while BOLP improves worst-group mIoU but degrades overall mIoU. This suggests that biome information can act both as a nuisance factor and as task-relevant context, particularly for dense land-cover prediction. Overall, \method should be viewed not only as a benchmark for ranking methods, but also as a diagnostic tool for understanding when ecological variation helps or hurts downstream RSFM transfer.
